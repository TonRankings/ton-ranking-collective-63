...
const gameCategories = [
  { id: 'tap2earn', name: 'Tap to Earn (T2E)', description: 'Simple tap games that reward players with cryptocurrency', icon: Ticket, color: 'bg-amber-500' },
  { id: 'play2earn', name: 'Play to Earn (P2E)', description: 'More complex games with cryptocurrency rewards for skilled play', icon: Gamepad, color: 'bg-emerald-500' },
  { id: 'competitive', name: 'Competitive Games', description: 'Games that involve competition between players for rewards', icon: Trophy, color: 'bg-blue-500' },
  { id: 'strategy', name: 'Strategy Games', description: 'Strategic games involving planning and resource management', icon: Award, color: 'bg-purple-500' },
  { id: 'collectible', name: 'Collectible Games', description: 'Games focused on collecting digital items and NFTs', icon: Coins, color: 'bg-rose-500' }
];
      ...
      {!selectedCategory ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-10 animate-stagger">
          {gameCategories.map((category) => (
            <button key={category.id} onClick={() => setSelectedCategory(category.id)} className="glass-card hover-scale rounded-xl overflow-hidden transition-all duration-300">
